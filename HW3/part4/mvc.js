...
myapp.view = {
	...
	sel_delButton: null,//刪除按鈕 選擇子(sel) 對應
	...
	//[5]12/25 新增「刪除自己」功能，將對象id綁在attr data-id 上、觸發以class=del毛略分類好了
	presenter: null,//
	init: function (){
		this.presenter = new myapp.Presenter();
		this.$addButton = $('#add');
		this.$saveButton = $('#save');
		this.sel_delButton = '.del:button';
		this.$result = $('#result');
	...
	bindEvents: function (){
		...
		this.$addButton.click(function (){//新增單一輸入欄位
			...
			view.$input_content.append("<label for='content_"+cnt+"'>"+cnt+"</label><input id='content_"+cnt+"' name='content_"+cnt+"'/><button type='button' class='del' data-id='content_"+cnt+"'>x</button><br><br>")
		...
		$(document).on("click",this.sel_delButton,(function (){//儲存單一輸入欄位
			console.log('%c mvc.js view.$delButton.click', 'background: #fff; color: #00f');
